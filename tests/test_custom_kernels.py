...
class TestCustomKernels():
    ...
    def test_point_elem_map(self, elem_dims, resolution):
        pts, elems, elems_perm = self.create_mesh(elem_dims-1, resolution)
        nr_points = pts.shape[0]
        point_elem_map = self.comp_point_elem_map(elems, nr_points)
        assert(point_elem_map.shape[0] == nr_points)

        for point_i in range(nr_points):
            contained_elems = np.where(point_i == elems)[0]
            assert(contained_elems.size > 0)
            assert(contained_elems.size == np.unique(elems, return_counts=True)[1][point_i])
            assert(np.all(np.unique(np.sort(contained_elems) == np.unique(point_elem_map[point_i]))))


    @pytest.mark.parametrize('elem_dims', [3, 4])
    @pytest.mark.parametrize('resolution', [5, 10, 15])
    def test_neighborhood_map(self, elem_dims, resolution):
        pts, elems, elems_perm = self.create_mesh(elem_dims-1, resolution)
        nr_points = pts.shape[0]

        nh_map = self.comp_neighborhood_map(elems, nr_points, elem_dims)
        point_elem_map = self.comp_point_elem_map(elems, nr_points)
        for point_i in range(nr_points):
            contained_elems = np.where(point_i == elems)[0]
            assert(contained_elems.size > 0)
            other_points = np.unique(elems[contained_elems])
            other_points = other_points[other_points != point_i]
            assert(np.all(np.unique(nh_map[point_i]) == other_points))


    @pytest.mark.parametrize('elem_dims', [3, 4])
    @pytest.mark.parametrize('resolution', [5, 10, 15])
    @pytest.mark.parametrize('nr_active_inds', [1, 20, 100])
    def test_perm_kernel_cpu(self, elem_dims, resolution, nr_active_inds):
        pts, elems, elems_perm = self.create_mesh(elem_dims-1, resolution)
        nr_pts = pts.shape[0]

        point_elem_map = self.comp_point_elem_map(elems, nr_pts)

        active_inds = np.random.choice(nr_pts, np.minimum(nr_active_inds, nr_pts), replace=False)
        active_elem_inds = np.unique(point_elem_map[active_inds].reshape([-1]))
        active_elems_perm = elems_perm[active_elem_inds]

        perm_mask = compute_perm_mask(np.ascontiguousarray(active_elems_perm[..., -1]), active_inds.astype(np.int32))
        ...
        assert(np.all(perm_mask == perm_mask_gt))
    ...
    @pytest.mark.parametrize('threads', [1, 256, 1024])
    def test_perm_kernel_gpu1(self, elem_dims, resolution, nr_active_inds, parallel_blocks, threads):
        pts, elems, elems_perm = self.create_mesh(elem_dims-1, resolution)
        nr_pts = pts.shape[0]

        elems_perm = cp.array(elems_perm)
        point_elem_map = cp.array(self.comp_point_elem_map(elems, nr_pts))
        active_inds = cp.array(np.random.choice(nr_pts, np.minimum(nr_active_inds, nr_pts), replace=False))
        active_elem_inds = cp.unique(point_elem_map[active_inds].reshape([-1]))
        active_elems_perm = elems_perm[active_elem_inds]

        perm_kernel = cp.RawKernel(compute_perm_kernel_str2.replace("{active_perms}", str(elem_dims)).replace("{parallel_blocks}", str(parallel_blocks)), 
                                    'perm_kernel')
        perm_mask_output = cp.zeros(shape=active_elems_perm.shape[0:2], dtype=bool)
        perm_kernel((parallel_blocks,), (threads,), (cp.ascontiguousarray(active_elems_perm[..., -1]), active_inds.astype(cp.int32), perm_mask_output, active_elems_perm.shape[0], active_inds.size))
        #Broadcasted version is the ground truth
        perm_mask_gt = np.any(active_elems_perm[..., -1, np.newaxis] == active_inds[np.newaxis, np.newaxis], axis=-1)
